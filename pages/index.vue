...
const valueBlocks = computed(() => [
  { title: t("portfolio.valuesLabels.environment"), body: values.value.work_environment, icon: "i-lucide-users" },
  { title: t("portfolio.valuesLabels.management"), body: values.value.management_preference, icon: "i-lucide-target" },
  { title: t("portfolio.valuesLabels.strengths"), list: values.value.strengths, icon: "i-lucide-zap" },
]);
...
<template>
  <div class="min-h-screen bg-neutral-950 text-neutral-50 overflow-hidden">
    ...
    <div class="fixed inset-0 pointer-events-none opacity-20" aria-hidden="true">
      <div class="absolute -top-40 -left-32 w-[500px] h-[500px] rounded-full blur-[120px] bg-brand-500/40 animate-[pulse-slow_8s_ease-in-out_infinite]" />
      <div class="absolute top-1/3 -right-20 w-[600px] h-[600px] rounded-full blur-[120px] bg-accent-500/40 animate-[pulse-slow_10s_ease-in-out_infinite]" />
      <div class="absolute bottom-0 left-1/4 w-[450px] h-[450px] rounded-full blur-[100px] bg-brand-400/30 animate-[pulse-slow_12s_ease-in-out_infinite]" />
    </div>

    <main class="relative max-w-7xl mx-auto px-5 sm:px-6 md:px-10 lg:px-12 py-12 md:py-20 space-y-20 md:space-y-28">
      <IndexHero
        :personal="personal"
        :headline="headline"
      ...
      <IndexExperienceEducation :experience="experience" :education="education" />
