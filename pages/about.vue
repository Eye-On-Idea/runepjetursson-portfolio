...
<template>
  <div class="min-h-screen bg-neutral-950 text-neutral-50 overflow-hidden">
    ...
    <div class="fixed inset-0 pointer-events-none opacity-15">
      <div class="absolute top-20 right-0 w-[500px] h-[500px] rounded-full blur-[120px] bg-brand-500/40 animate-[pulse-slow_10s_ease-in-out_infinite]" />
      <div class="absolute bottom-0 left-0 w-[600px] h-[600px] rounded-full blur-[120px] bg-accent-500/30 animate-[pulse-slow_12s_ease-in-out_infinite]" />
    </div>

    <div class="relative max-w-6xl mx-auto px-5 sm:px-6 md:px-10 lg:px-12 py-12 md:py-20 space-y-16 md:space-y-24">
      <!-- Header -->
      <header class="space-y-6 text-center lg:text-left">
        ...
        <p class="text-xl md:text-2xl text-neutral-200 leading-relaxed max-w-3xl mx-auto lg:mx-0">{{ headline.tagline }}</p>
      </header>
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <h2 class="text-2xl md:text-3xl font-bold text-white">{{ t("portfolio.about.sections.background") }}</h2>
        </div>
      ...
      <section class="space-y-8">
        <h2 class="text-3xl md:text-4xl font-bold text-white text-center lg:text-left">
          {{ t("portfolio.sections.values") }}
        </h2>
        <div class="grid md:grid-cols-3 gap-6 lg:gap-8">
          <GlassCard
            v-for="block in [
              { title: t('portfolio.valuesLabels.environment'), body: values.work_environment, icon: 'i-lucide-users' },
              { title: t('portfolio.valuesLabels.management'), body: values.management_preference, icon: 'i-lucide-target' },
              { title: t('portfolio.valuesLabels.strengths'), body: values.strengths.join(' • '), icon: 'i-lucide-zap' },
            ]"
            :key="block.title"
          ...
          >
            ...
            <p class="text-neutral-200 leading-relaxed text-base">{{ block.body }}</p>
          </GlassCard>
        </div>
      </section>
      ...
      <!-- Skills Section -->
      <section class="space-y-8">
        <h2 class="text-3xl md:text-4xl font-bold text-white text-center lg:text-left">{{ t("portfolio.about.sections.skills") }}</h2>
        <div class="grid md:grid-cols-3 gap-6 lg:gap-8">
          <GlassCard
            :displacement-scale="32"
            :blur-amount="0.06"
          ...
          >
            <div class="flex items-center gap-3">
              ...
              <h3 class="text-lg font-bold text-white">{{ t("portfolio.sections.skills.core") }}</h3>
            </div>
            <ul class="space-y-3 text-neutral-200">
              <li v-for="item in skills.core" :key="item" class="flex gap-3 items-center">
          ...
          >
            <div class="flex items-center gap-3">
              ...
              <h3 class="text-lg font-bold text-white">{{ t("portfolio.sections.skills.tech") }}</h3>
            </div>
            <ul class="space-y-3 text-neutral-200">
              <li v-for="item in skills.technologies" :key="item" class="flex gap-3 items-center">
                <span class="w-1.5 h-1.5 rounded-full bg-brand-400 shrink-0"></span>
                <span>{{ item }}</span>
              </li>
            </ul>
          ...
          >
            <div class="flex items-center gap-3">
              ...
              <h3 class="text-lg font-bold text-white">{{ t("portfolio.sections.skills.tools") }}</h3>
            </div>
            <ul class="space-y-3 text-neutral-200">
              <li v-for="item in [...(skills.tools || []), ...(skills.methods || [])]" :key="item" class="flex gap-3 items-center">
                <span class="w-1.5 h-1.5 rounded-full bg-brand-400 shrink-0"></span>
                <span>{{ item }}</span>
              </li>
            </ul>
      ...
      <!-- Projects Section -->
      <section class="space-y-8">
        <h2 class="text-3xl md:text-4xl font-bold text-white text-center lg:text-left">{{ t("portfolio.about.sections.cases") }}</h2>
        <div class="space-y-6">
          <GlassCard
          ...
          >
            <div class="space-y-2">
              <h3 class="text-xl md:text-2xl font-bold text-white group-hover:text-brand-300 transition-colors">{{ project.name }}</h3>
            ...
            <p class="text-neutral-200 leading-relaxed">{{ project.summary }}</p>
          </GlassCard>
        </div>
      </section>
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <h2 class="text-2xl md:text-3xl font-bold text-white">{{ t("portfolio.about.sections.approach") }}</h2>
        </div>
        <p class="text-neutral-200 text-lg leading-relaxed">{{ values.approach }}</p>
      </GlassCard>

      <!-- CTA -->
      <div class="text-center space-y-6 pt-8">
        <h3 class="text-2xl md:text-3xl font-bold text-white">Ready to work together?</h3>
        <NuxtLink
          to="/contact"
